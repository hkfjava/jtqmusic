                        ...
                        <div class="row">
                            <div class="col-sm-10">
                                <div class="panel wrapper-lg">
                                    <div class="row">
                                        <div class="col-sm-3">
                                            <img id="img_album_img" src="${singer.singer_imgUrl}" class="img-full r r-2x">
                                        </div>
                                        <div class="col-sm-7">
                                            <h2 class="m-t-none text-black">${singer.singer_name}</h2>
                                            <div class="clearfix m-b-lg">
                                                <%--<a href="#" class="thumb-sm pull-left m-r">
                                                    <img src="images/a0.png" class="img-circle">
                                                </a>--%>
                                                <div class="clear">
                                                    <a href="${pageContext.request.contextPath}/singer/findAll.do?region_id=${region.region_id}" class="text-info">国籍：${region.region_name}</a>
                                                    <div href="#" class="text-info"></div>
                                                    <div class="block text-muted">${singer.singer_heat} 热度 / ${singer.singer_collCount} 粉丝</div>
                                                </div>
                                            </div>
                                            <div class="m-b-lg">
                                                <span  onclick="coll_singer(this,${singer.singer_id})" coll_flag="0" class="btn btn-info">收藏</span> <a href="#" class="btn btn-default">3 评论</a>
                                            </div>
                                            <div>
                                                标签: <a href="${pageContext.request.contextPath}/singer/findAll.do?classify_id=${classify.classify_id}" class="badge bg-light">${classify.classify_name}</a> <a href="${pageContext.request.contextPath}/singer/findAll.do?region_id=${region.region_id}" class="badge bg-light">${region.region_name}</a> <a href="#" class="badge bg-light">
                                                <c:if test="${singer.singer_sex == 0}">男</c:if><c:if test="${singer.singer_sex == 1}">女</c:if>
                                                 歌手</a>
                                    ...
                                    <section class="panel panel-default">
                                        <header class="panel-heading bg-light">
                                            <ul class="nav nav-tabs nav-justified" id="ul_selectupload">
                                                <li class="active"><a href="#home" data-toggle="tab" onclick="getSongs()">歌曲</a></li>
                                                <li><a href="#profile" data-toggle="tab" onclick="getAlbum()">专辑</a></li>
                                                <li><a href="#messages" data-toggle="tab">详情</a></li>
                                                <li><a href="#like" data-toggle="tab" onclick="getSinger()">相似歌手</a></li>
                                                <li><a href="#settings" data-toggle="tab" onclick="getMV()">MV</a></li>
                                            </ul>
                                        </header>
                                        <div class="panel-body">
                                            <div class="tab-content">
                                                <li class="list-group-item">
                                                    <div onclick="play_allSongs()" class="btn btn-info">全部加入播放列表 </div>&nbsp;&nbsp;&nbsp;&nbsp; <a href="#" class="btn btn-dark">下载全部 </a> &nbsp;&nbsp;&nbsp;&nbsp;<a href="#" class="btn btn-default">3 评论</a>
                                                </li>
                                                <%-- 歌曲 --%>
                                                ...
                                                <div class="tab-pane" id="messages">
                                                    <p>姓名：${singer.singer_name}</p>
                                                    <p>国籍：${region.region_name}</p>
                                                    <p>性别：<c:if test="${singer.singer_sex == 0}">男</c:if><c:if test="${singer.singer_sex == 1}">女</c:if></p>
                                                    <p>生日：${singer.singer_birth_str}  </p>
                                                    <p>出道时间：${singer.singer_timeOut_str}</p>
                                                </div>
                                                ...
                                                <div class="tab-pane" id="settings">MV</div>
                                            </div>
                                        </div>
                                    </section>
...
<script type="text/javascript">
    ...
    // 收藏歌手函数
    function coll_singer(obj,singer_id) {
        console.log("点击收藏歌手coll_album,歌手id：" + singer_id)
        ...
        if ($(obj).attr("coll_flag") == 0){
            ...
            $.ajax({
                ...
                type: "POST",//方法类型
                dataType: "json",//预期服务器返回的数据类型
                url: '${pageContext.request.contextPath}/user/coll.do?singer_id=' + singer_id,//url
                success: function (result) {
                    console.log(result);//打印服务端返回的数据(调试用)
                    if (result = 101) {
                        // 添加class
            ...
            console.log("异步取消收藏歌手");

            $.ajax({
                //几个参数需要注意一下
                type: "POST",//方法类型
                dataType: "json",//预期服务器返回的数据类型
                url: '${pageContext.request.contextPath}/user/coll_cancel.do?singer_id=' + singer_id,//url
                success: function (result) {
                    console.log(result);//打印服务端返回的数据(调试用)
                    if (result = 101) {
                        // 添加class
